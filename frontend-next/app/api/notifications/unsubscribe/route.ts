...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await client.query('DELETE FROM notification_ids WHERE phone_number = $1', [sanitizedPhone]);
    if (result.rowCount === 0) {
      return NextResponse.json({ error: 'Phone number not found.' }, { status: 404 });
    }
    
    console.log(`Successfully unsubscribed phone number: ${sanitizedPhone}`);
    return NextResponse.json({ success: true, message: `Unsubscribed: ${sanitizedPhone}` });
  } catch (error: any) {
